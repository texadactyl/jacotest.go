 ...
 *        monitorenter, monitorexit, wait, notify, notifyAll, interrupt
 ...
 Expected output something like this:
        ...
        === Test 7: Recursive Locking ===
        ok Recursive counter ::: expected = observed = 100
        === Test 8: High Contention Single Object ===
        ok Contention counter ::: expected = observed = 10000
...
public class main {
    private static final int PRODUCER_COUNT = 64;
    private static final int CONSUMER_COUNT = 64;
    private static final int ITEMS_TO_PRODUCE = 1000;
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        // Test 7: Recursive locking (thin -> fat lock inflation)
        System.out.println("=== Test 7: Recursive Locking ===");
        RecursiveCounter recursiveCounter = new RecursiveCounter();
        Thread[] recursiveThreads = new Thread[4];
        for (int i = 0; i < 4; i++) {
            recursiveThreads[i] = new RecursiveCounterThread(recursiveCounter, 25);
            recursiveThreads[i].start();
        }
        
        for (Thread t : recursiveThreads) {
            t.join();
        }
        
        // 4 threads x 25 iterations = 100
        errorCount += Checkers.checker("Recursive counter", 
            100, recursiveCounter.getCount());
        
        // Test 8: High contention on a single object
        System.out.println("=== Test 8: High Contention Single Object ===");
        ContentionCounter contentionCounter = new ContentionCounter();
        Thread[] contentionThreads = new Thread[10];
        for (int i = 0; i < 10; i++) {
            contentionThreads[i] = new ContentionThread(contentionCounter, 1000);
            contentionThreads[i].start();
        }
        ...
        // 10 threads x 1000 iterations = 10000
        errorCount += Checkers.checker("Contention counter", 
            10000, contentionCounter.getCount());
...
/**
 * Tests recursive locking (reentrant synchronized blocks).
 * Three nested synchronized blocks on the same object force:
 *   depth 1 - thin lock
 *   depth 2 - thin -> fat inflation
 *   depth 3 - fat lock reentrant acquisition
 */
class RecursiveCounter {
    ...
    public void increment() {
        synchronized (lock) {                // depth 1 - thin lock
            synchronized (lock) {            // depth 2 - inflate to fat
                synchronized (lock) {        // depth 3 - fat reentrant
                    count++;
                }
            }
        }
...
class RecursiveCounterThread extends Thread {
    private final RecursiveCounter counter;
    private final int iterations;
    
    RecursiveCounterThread(RecursiveCounter counter, int iterations) {
        this.counter = counter;
        this.iterations = iterations;
    }
...
/**
 * Tests high contention on a single lock object.
 * Many threads hammering the same object rapidly to stress
 * the thin->fat inflation and fat lock contention paths.
 */
class ContentionCounter {
    private final Object lock = new Object();
    private int count = 0;
...
class ContentionThread extends Thread {
    private final ContentionCounter counter;
    private final int iterations;
    
    ContentionThread(ContentionCounter counter, int iterations) {
        this.counter = counter;
        this.iterations = iterations;
    }
