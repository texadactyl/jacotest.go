...
public final class fannkuch implements Runnable
{
    ...
    private final void print_30_permut()
    {
        ...
        final int[] permutation = new int[nthreads];
        ...
        final int[] perm_remain = new int[nthreads];
        for ( int i = 1; i <= nthreads; i++ )
            perm_remain[i -1] = i;
        
        int numPermutationsPrinted = 1;
        for ( int pos_right = 2; pos_right <= nthreads; pos_right++ )
        {
    ...
    public void run()
    {
        final int[] permutation = new int [nthreads];
        final int[] perm_remain = new int [nthreads];
        final int[] perm_flip = new int [nthreads];

        int pos_right;
        while ((pos_right = remain_task.getAndIncrement()) < (nthreads - 1))
        {
            int flip_max = 0;
            ...
            permutation[pos_right] = (nthreads - 1);
            permutation[nthreads - 1] = (pos_right);

            for (int i = 1; i <= nthreads; i++)
                perm_remain[i - 1] = i;

            int pos_left = nthreads - 2;
            while (pos_left < nthreads - 1)
            {
                ...
                if (--perm_remain[pos_left] > 0)
                {
                    ...
                    if ((permutation[0] != 0) && (permutation[nthreads - 1] != (nthreads - 1)))
                    {
                        System.arraycopy(permutation, 0, perm_flip, 0, nthreads);
