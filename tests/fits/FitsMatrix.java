...
public class FitsMatrix extends FitsData {
    ...
    public FitsMatrix(FitsHeader header, DataInput file, boolean sflag) throws FitsException {
        ...
        if (type != Fits.IMAGE) {
            throw new FitsException("Wrong header type", FitsException.HEADER);
        }

        FitsKeyword kw = header.getKeyword("BITPIX");
        if ((kw == null) || (kw.getType() != FitsKeyword.INTEGER)) {
            throw new FitsException("Invalid or missing BITPIX", FitsException.HEADER);
        }
        ...
        try {
            switch (dataFormat) {
                ...
                case Fits.DOUBLE:
                    ...
                    break;
                default:
                    throw new FitsException("Invalid data format",
                            FitsException.DATA);
            }
