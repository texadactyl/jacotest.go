...
public class main {
    ...
    private static int tryIt(int ix, String ba1Expected, String ba2Expected, String ba3Expected) {
        ...
        int errorCount = 0;
        String observed;
        MessageDigest mdAlpha, mdBeta, mdGamma;
        byte[] ba1Bytes, ba2Bytes, ba3Bytes, ba101Bytes;
        String label;
        
        try {
        
            mdAlpha = MessageDigest.getInstance(algorithms[ix]);
            ba101Bytes = mdAlpha.digest(inBytes1);
            
            mdGamma = MessageDigest.getInstance(algorithms[ix], providerName);
            ba1Bytes = mdAlpha.digest(inBytes1);
            
            mdBeta = (MessageDigest) mdAlpha.clone();
            ...
            errorCount += Checkers.checker("isEqual", true, MessageDigest.isEqual(ba1Bytes, ba101Bytes));
            
            mdBeta = MessageDigest.getInstance(algorithms[ix]);
            ba2Bytes = mdBeta.digest(inBytes2);
            ...
            errorCount += Checkers.checker("isEqual", false, MessageDigest.isEqual(ba1Bytes, ba2Bytes));
            
            mdBeta = MessageDigest.getInstance(algorithms[ix]);
            mdBeta.update(inBytes1);
            ba3Bytes = mdBeta.digest(inBytes2);
            ...
            mdGamma.update(inBytes1);
            ba3Bytes = mdGamma.digest(inBytes2);
            label = String.format("ba3Bytes with %s - mdGamma", algorithms[ix]);
            String ba3Hexed = HexDump.bytesToHex(ba3Bytes);
            errorCount += Checkers.checker(label, ba3Expected, HexDump.bytesToHex(ba3Bytes));
            
            String whichAlgo = mdGamma.getAlgorithm();
            errorCount += Checkers.checker("get algorithm", algorithms[ix], whichAlgo);
            
            int diglen = mdGamma.getDigestLength();
            errorCount += Checkers.checker("get digest length", ba3Bytes.length, diglen);
            
            Provider provider = mdGamma.getProvider();
            String str = provider.getName();
            errorCount += Checkers.checker("get provider name", providerName, str);
            
            mdGamma.reset();
            whichAlgo = mdGamma.getAlgorithm();
            errorCount += Checkers.checker("get algorithm after reset", algorithms[ix], whichAlgo);
    ...
    public static void main(String[] args) {
        ...
        if (jvmPgmName.equals("java"))
            ...
            providerName = "GoSecurityProvider";
