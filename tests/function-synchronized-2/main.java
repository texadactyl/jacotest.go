...
/**
 * Shared object with synchronized methods
 */
class Counter {
    private long value = 0;
    private static long staticValue = 0;
    ...
    public static synchronized void incrementStatic() {
        ...
        if ((staticValue & 0xFF) == 0) {
            try {
                Counter.class.wait(1);
            } catch (InterruptedException ignored) {
            }
        }
    }

    public static synchronized long getStatic() {
        return staticValue;
    }
}
...
class Worker extends Thread {
    ...
    Worker(Counter counter, int id, int iterations, int yield_control) {
    ...
    public void run() {
        for (int ix = 0; ix < iterations; ix++) {
            ...
            /*
            Calling Thread.yield() too often would:
            - Kill throughput
            - Turn the benchmark into a scheduler test instead of a lock test

            Calling it occasionally:
            - Forces context switches
            - Increases lock contention
            - Exercises monitor enter/exit paths more aggressively

            This line ensures:
            - Yield happens regularly
